...
def get_start_date():
  ...
  print()
  print('The date must be greater then Jan 1st, 2018')
...
# save lines to new variable to improve efficieny of functions
...
def get_same_or_newer(start_date):
  ...
  #we want all employees that started at the same date or the closest
  # newer date.  To calulate that, we go through all the data and find
  # the employees that started on the smallest date that's equal to or
  # bigger than the given start date.
  min_date = datetime.datetime.today()
  min_date_employees = []
  for row in reader:
    row_date = datetime.datetime.strptime(row[3], '%Y-%m-%d')
    
    # if this date is smaller than the one we're looking for, we skip this row
